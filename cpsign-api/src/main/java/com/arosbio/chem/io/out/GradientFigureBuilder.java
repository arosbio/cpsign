...
public class GradientFigureBuilder extends MoleculeFigureBuilder {
	...
	public MoleculeFigure build(IAtomContainer mol, Map<?,Double> gradient){
		Map<IAtom, Double> grad = new HashMap<>();
		for (Map.Entry<?, Double> atom: gradient.entrySet()){
		...
		int finalFigureWidth=0, finalFigureHeight=0;
		...
		if (explicitFigureSizeBeenSet()) {
			...
			molImg = depictor.depict(mol, grad);
		} else {
