...
/**
 * Uses the negative probability for each class (as the probabilities are how <b>conforming</b> the
 * example is for that class).
 * 
 * @author staffan
 *
 */
public class InverseProbabilityNCM implements NCMMondrianClassification {

	private static final Logger LOGGER = LoggerFactory.getLogger(InverseProbabilityNCM.class);
	private static final Version VERSION = new Version(1,0,0);
	public final static String IDENTIFIER="InverseProbability";
	...
	public final static String DESCRIPTION = "Nonconformity measure for classification. This NCM requires a scoring algorithm "
			+ "that can output probability-estimates. The negative of the probability is used as the nonconformityscore for an object.";
	...
	public InverseProbabilityNCM() {}

	public InverseProbabilityNCM(PseudoProbabilisticClassifier classifier) {
		this.classifier = classifier;
	}

	@Override
	public InverseProbabilityNCM clone() {
		if (classifier != null)
			return new InverseProbabilityNCM(classifier.clone());
		return new InverseProbabilityNCM();
	}
	...
	public void setModel(Classifier probabilisticModel) throws IllegalArgumentException {
		if (probabilisticModel instanceof PseudoProbabilisticClassifier)
			...
			throw new IllegalArgumentException("Probability estimates NCM requires a probablistic model");
	}
	...
	public void trainNCM(List<DataRecord> data) throws IllegalArgumentException {
		...
		LOGGER.debug("Finished training NCM {}",IDENTIFIER);
	}
	...
	public Map<Integer, Double> calculateNCS(FeatureVector example) 
		...
		// we need to negate the probabilities (higher probability => more CONFORMING) 
		Map<Integer, Double> probs = classifier.predictProbabilities(example);
		for (int label : probs.keySet()) {
			probs.put(label, -1*probs.get(label));
		}
...
//			if (classifier.getParameters() != null) {
//				svmParamsSavePath = path + NCMUtils.SCORING_MODEL_FILE + NCMUtils.MODEL_PARAMS_FILE_ENDING;
//				AlgorithmUtils.saveModelParamsToStream(sink, svmParamsSavePath, classifier.getParameters());
//			}
		}
...
//			AlgorithmUtils.loadAlgorithmParams(src, params.get(PropertyFileSettings.NCM_SCORING_PARAMETERS_LOCATION).toString(), classifier.getParameters());
