 ...
 * nonconf = |<i>y</i> - <i>y_hat</i>|/(<i>μ_hat</i>+<i>β</i>) , <i>μ_hat</i> = predicted error by error model, <i>β</i> &gt;= 0 "smoothing factor"
 * <p>
 ...
 * @author staffan
 */
public class NormalizedNCM implements NCMRegression {

	private static final Version VERSION = new Version(1, 1, 0);
	private static final Logger LOGGER = LoggerFactory.getLogger(NormalizedNCM.class);
	
	public static final String IDENTIFIER="Normalized";
	...
	public final static String DESCRIPTION = "Nonconformity measure for regression. Residuals found for the calibration "
			...
			+ "The error-model is trained on the absolute values of the residuals in contrast with the " + LogNormalizedNCM.IDENTIFIER + " NCM that "
			+ "trains on the logarithm of the residuals. "
	...
	private double beta=NCMRegression.DEFAULT_BETA_VALUE;
	...
	public NormalizedNCM() {}

	public NormalizedNCM(Regressor model) {
		this(model, model.clone());
	}

	public NormalizedNCM(Regressor model, Regressor errorModel) {
		if (model == errorModel)
			throw new IllegalArgumentException("scoring model and error model cannot be the same instance");
		Objects.requireNonNull(model, "Regressor model cannot be null");
		this.model = model;
		if (errorModel == null){
			LOGGER.debug("No explicit error model given, using the same settings as the scoring model");
	...
	public NormalizedNCM(Regressor model, Regressor errorModel, double beta) {
		this(model, errorModel);
		this.setBeta(beta);
	}

	public NormalizedNCM clone() {
		NormalizedNCM clone = new NormalizedNCM();
		if (model != null)
	...
	public void setBeta(double beta) {
		if (beta <0)
			throw new IllegalArgumentException(NCMRegression.INVALID_BETA_ERR_MSG);
		this.beta = beta;
	}
	...
	public List<ConfigParameter> getConfigParameters() {
		...
		// Beta parameter
		params.add(new NumericConfig.Builder(CONFIG_BETA_PARAM_NAMES, NCMRegression.DEFAULT_BETA_VALUE)
	...
	public void trainNCM(List<DataRecord> data) {
		...
		for (DataRecord rec : data) {
			...
			errorRecs.add(new DataRecord(Math.abs(rec.getLabel()-y_hat), rec.getFeatures()));
		}
