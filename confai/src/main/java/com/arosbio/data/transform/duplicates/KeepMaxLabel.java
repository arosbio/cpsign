...
public class KeepMaxLabel implements DuplicatesResolverTransformer {
	
	public static final String DESCRIPTION = "For regression problems - use the maximum target value of the duplicate records.";

	private static final String NAME = "KeepMaxLabel";

	private static final Logger LOGGER = LoggerFactory.getLogger(KeepMaxLabel.class);
	...
	@Override
	public KeepMaxLabel transformInPlace(boolean inPlace) {
		this.inPlace = inPlace;
		return this;
	}
	
	public KeepMaxLabel clone() {
		return new KeepMaxLabel();
	}
	...
	public SubSet transform(SubSet data) throws IllegalStateException {
		...
		for (DuplicateEntry entry: dups) {
			entry.getRemainingRecord().setLabel(Collections.max(entry.getLabels()));
		}
