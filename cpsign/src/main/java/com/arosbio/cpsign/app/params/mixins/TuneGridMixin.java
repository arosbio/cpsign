/*
 * Copyright (C) Aros Bio AB.
 *
 * CPSign is an Open Source Software that is dual licensed to allow you to choose a license that best suits your requirements:
 *
 * 1) GPLv3 (GNU General Public License Version 3) with Additional Terms, including an attribution clause as well as a limitation to use the software for commercial purposes.
 *
 * 2) CPSign Proprietary License that allows you to use CPSign for commercial activities, such as in a revenue-generating operation or environment, or integrate CPSign in your proprietary software without worrying about disclosing the source code of your proprietary software, which is required if you choose to use the software under GPLv3 license. See arosbio.com/cpsign/commercial-license for details.
 */
package com.arosbio.cpsign.app.params.mixins;

import java.util.LinkedHashMap;

import com.arosbio.cpsign.app.ExplainArgument.TuneParamsInfo;
import com.arosbio.cpsign.app.utils.ParameterUtils;
import com.arosbio.cpsign.app.utils.ParameterUtils.ArgumentType;

import picocli.CommandLine.Option;

public class TuneGridMixin {
	
	@Option(names = {"-g", "--grid"},
			description = "Specify which parameters that should be part of the parameter grid, specified using syntax "
					+ "-g <KEY>=<VALUES> or --grid <KEY>=<VALUES>, e.g., -g=COST=1,10,100 or --grid=Gamma=b2:-8:-1:2. Run "+
					ParameterUtils.RUN_EXPLAIN_ANSI_ON+"explain " + TuneParamsInfo.SUB_NAME + ParameterUtils.ANSI_OFF+" for further details. "
							+ "Adding more than one parameter is achieved with separate parameter calls, e.g. -g c=10,100 -g gamma=0.001, 0.01.",
			paramLabel = ArgumentType.TUNE_KEY_VALUE,
			mapFallbackValue = "default",
			arity = "1..*",
			required = true)
	public LinkedHashMap<String,String> paramGrid;
	
}
